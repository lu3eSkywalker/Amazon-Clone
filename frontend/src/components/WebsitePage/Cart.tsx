...
const Cart = () => {
    ...
    const fetchCartItems = async(): Promise<void> => {
        ...
        try {
            const cartItemPromises = userDetail.cart.map(async (cartElement) => {
                const cartItems = await fetch (`${process.env.REACT_APP_BASE_URL}/getproductbyid/${cartElement}`, {
                    method: "GET",
                })
    ...
    const deleteCartItems = async(productId: string): Promise<void> => {
      ...
      try {
        const cartItemremove = await fetch(`${process.env.REACT_APP_BASE_URL}/deletecartItems`, {
            method: 'POST',
            headers: {
...
{
cartItemInfo.length === 0 ? <div className='font-bold text-xl flex justify-center my-[250px]'>Looks, Like Your cart is empty. Continue Browsing <button className='text-white bg-gray-700 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-700 dark:hover:bg-gray-800 dark:focus:ring-gray-700 dark:border-gray-700' onClick={() => navigate('/frontpage')}>HomePage</button></div> : 

<button  onClick={() => navigate('/fullcartorder')}
