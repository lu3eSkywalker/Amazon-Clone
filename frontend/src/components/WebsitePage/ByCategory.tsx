...
const ByCategory = () => {
    ...
    const getProducts = async(category : string): Promise<void> => {
        try {
            const savedResponse = await fetch(`${process.env.REACT_APP_BASE_URL}/bycategory/${category}`, {
                method: "GET",
            })
    ...
    const getProductsPagination = async(page: number, limit: number): Promise<void> => {
        try {
            const savedResponse = await fetch(`${process.env.REACT_APP_BASE_URL}/bycategory/${category}?page=${page}&limit=${limit}`, {
                method: 'GET',
            });
            const response = await savedResponse.json();
